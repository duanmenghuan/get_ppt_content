        ...
        try:
            if text_length != 0:
                if is_cover_slide(slide_index):
                    ...
                    return '目录标题' if text_length <= 12 and not text.isdigit() else ' '
        ...
        for shape in shapes:
            ...
            if shape.has_text_frame:
                ...
                if not text.isdigit() and len(text) <=1:
                    continue
                if any(keyword in text for keyword in ['前  言', '前言', '目录', '目\n录', '目 录','前 言'] if len(text) <=4):
                    continue
                elif contains_number_and_check_length(shape,text) or is_english_with_spaces(text):
                    continue
                if ls:
                    type_ = '标题'
                    hint_ext = ls[1]
                ...
                for subshape in subshapes:
                    if subshape.has_text_frame:
                        ...
                        # text = text_frame.text.strip()
                        # text = ''.join(text.split())
                        #print("GroupShape", text,subshape.shape_type)
                        if  not text.isdigit() and len(text) <=1:
                            continue
                        if any(keyword in text for keyword in ['前  言', '前言', '目录', '目\n录','前 言'] if len(text) <=4):
                            continue
                        elif contains_number_and_check_length(subshape,text) or is_english_with_spaces(text):
                             continue
                        if ls:
                            type_ = '标题'
                            hint_ext = ls[1]
